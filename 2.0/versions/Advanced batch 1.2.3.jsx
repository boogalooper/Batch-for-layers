...
msgSave= "Скрипт остановлен при сохранении файла"
msgNoLayer = "Скрипт не может активировать слой, ранее помеченный для обработки"
captLrSet="управляющая группа"
captInit="инициализация..."
captBatch = "обработка..."
...
{  var atnLayersToDo = new collectLayers (atnSettings, atnExclude)
    Batch (atnSettings, atnLayersToDo)}
}
}
...
progressWindow.show();
progressWindow.stProgress.text = ( captLrSet+ " '" + controlLayerSet.name + "'' - " + lrSetColor);
...
if (IsControlGroupYetInArray (atnSettings, lrSetColor) < 0 && lrSetColor != "none")
    {
        if (controlLayerSet.visible !=false)
...
function collectLayers (AtnSettings, AtnExclude)
{
    ...
    var progressWindow = createProgressWindow(msgBatchInit, undefined, 0, doc.artLayers.length+doc.layerSets.length+app.documents.length ,undefined,false);
    progressWindow.show();
    progressWindow.isDone = false;
    progressWindow.stProgress.text = captInit
...
progressWindow.show();
progressWindow.isDone = false;
progressWindow.stProgress.text = captBatch
...
for (var i=atnSettings.length-1; i>=0; i--)
{
    ...
    {
        if (atnSettings[i].Color == atnLayersToDo[x].color)
       {
        ...
        if (atnSettings[i].Color == atnLayersToDo[x].color && errBreak==false)
        {
            try
            ...
            if (atnSettings[i].MakeSnapshotVisible==true)
            {
                 ...
                 { try {revertNamedSnapshot (GroupCaption)} catch (e) {app.activeDocument.activeHistoryState =app.activeDocument.historyStates[0]}
                   app.activeDocument  = doc
    ...
    if (atnSettings[i].MakeSnapshotVisible==true) revertNamedSnapshot (GroupCaption)
 }
