...
lrCaption = "Объединять группы слоев"
...
createControlLayer (controlLayerSet, lrCaption, atnParams.params.FlattenLayerSetVisible)
createControlLayer (controlLayerSet, snapCaption, atnParams.params.MakeSnapshorVisible)
...
if (controlLayerSet.artLayers.length >3)
{ counter = controlLayerSet.artLayers.length-3
    for (var i =0; i < counter; i++)
    ...
    {
      if (controlLayerSet.artLayers.length ==2) createControlLayer (controlLayerSet, actCaption, atnParams.params.DefaultActionNameVisible).move (subControlLayerSet, ElementPlacement.PLACEBEFORE)
...
if (IsControlGroupYetInArray (atnSettings, lrSetColor) < 0 && lrSetColor != "none")
    {
     ...
     tmp.MakeSnapshorVisible=controlLayerSet.artLayers[2].visible
     tmp.FlattenLayerSetVisible=controlLayerSet.artLayers[1].visible
     tmp.JPG= subControlLayerSet.artLayers[0].visible
     tmp.TIF=subControlLayerSet.artLayers[1].visible
     tmp.PSD=subControlLayerSet.artLayers[2].visible
...
for (var i=atnSettings.length-1; i>=0; i--)
{
    ...
    for (var x=atnLayersToDo.length-1; x>=0; x--)
    {
        ...
        if (atnSettings[i].Color == atnLayersToDo[x].color && atnSettings[i].ControlSetVisible==true && errBreak==false)
        {
            ...
            if (atnSettings[i].FlattenLayerSetVisible==true)
            {
                if (atnLayersToDo[x].parent != 0)
                {
                    selectLayerById[atnLayersToDo[x].parent]
            ...
            if (atnSettings[i].MakeSnapshorVisible==true)
            {
            ...
            if (atnLayersToDo[x].parent != 0 && atnSettings[i].FlattenLayerSetVisible==false ) {
             app.activeDocument  = doc
            doc.activeLayer.parent.visible =lrSetVisible
            doc.activeLayer.visible= lrVisible}
         }
    }
 }
...
// класс xml-based хранения настроек программы
// Written by Tom Ruark (image processor)
...
this.LoadParamsFromDisk = function() {
	...
	this.InitParams = function() {
		...
		params["MakeSnapshorVisible"] = true;
		params["FlattenLayerSetVisible"] = false;
		params["JPG"] = true;
		params["PSD"] = false;
		params["TIF"] = false;
		params["Color"] = "red";
