...
atnPanel=["Пакетная обработка", "Batch processing"]; atnPanel=atnPanel[atnXML.settings.langId]
cptColorPanelLrs = ["Выбор слоев для обработки", "Select layers for batch processing"]; cptColorPanelLrs =cptColorPanelLrs[atnXML.settings.langId]
cptColorPanelGroups  = ["Выбор групп для обработки", "Select groups for batch processing"]; cptColorPanelGroups =cptColorPanelGroups[atnXML.settings.langId]
cptAutoSearchLrs = ["все отмеченные цветом слои", "all layers with color labels"]; cptAutoSearchLrs = cptAutoSearchLrs[atnXML.settings.langId]
cptAutoSearchGroups = ["все отмеченные цветом группы", "all groups with color labels"]; cptAutoSearchGroups = cptAutoSearchGroups[atnXML.settings.langId]
...
cptSaveAfter=["cохранять как...", "save as..."]; cptSaveAfter=cptSaveAfter[atnXML.settings.langId]
cptUseAtn=["запуск операции для каждого слоя:", "play action for each layer:"];cptUseAtn=cptUseAtn[atnXML.settings.langId]
cptSet=["Группа:", "Action set:"];cptSet=cptSet[atnXML.settings.langId]
cptAtn=["Операция:", "Action:"];cptAtn=cptAtn[atnXML.settings.langId]
...
cptOk = ["Обработка", "Batch processing"];cptOk =cptOk[atnXML.settings.langId]
...
cptAllDocs = ["возвращать к исходному все документы", "back to initial state all opened documents"];cptAllDocs =cptAllDocs[atnXML.settings.langId]
cptJpg= ["Качество jpg:", "Quality of jpg"];cptJpg=cptJpg[atnXML.settings.langId]
cptFlatten = ["объединять слои (psd, tif)", "fatten layers (psd, tif)"];cptFlatten =cptFlatten[atnXML.settings.langId]
...
numOneGroup=["группа", "group"];numOneGroup=numOneGroup[atnXML.settings.langId]
numTwoGroups=["группы", "groups"];numTwoGroups=numTwoGroups[atnXML.settings.langId]
numManyGroups=["групп", "groups"];numManyGroups=numManyGroups[atnXML.settings.langId]
...
cptType =["Обработка слоев следующих типов:","eng"];cptType=cptType[atnXML.settings.langId]
cptGraph = ["графические слои","graphic layers"];cptGraph=cptGraph[atnXML.settings.langId]
cptText = ["текстовые слои","text layers"];cptText=cptText[atnXML.settings.langId]
cptGroup = ["группы","groups"];cptGroup=cptGroup[atnXML.settings.langId]
cptOther = ["корректирующие слои","adjustment layers"];cptOther=cptOther[atnXML.settings.langId]
cptEng = ["Английский", "Eng"], cptEng=cptEng[atnXML.settings.langId]
cptRus= ["Русский","Rus"], cptRus= cptRus[atnXML.settings.langId]
...
function buildWindow ()
{
    ...
    txtSet.preferredSize.height = txtAtn.preferredSize.height = 22
    ...
    var checkboxEng = g6.add ("radiobutton",undefined, cptEng)
    var checkboxRus= g6.add ("radiobutton",undefined, cptRus)
    ...
    var dropboxType = p4.add ("dropdownlist",undefined, [cptGraph, cptText, cptGroup, cptOther])
    p4.add ("group")
...
//tpanel.onChange = function () {if(tpanel.label == 1) {w.onShow(true);tpanel.label =0;atnXML.SaveParamsToDisk ()}}
checkboxBackground.onClick = function (){atnXML.settings.doBackground= this.value; w.onShow(true)}//tpanel.label=1}
...
checkboxColor.onClick = function ()
{
 ...
 //if (btnDo.enabled) { {checkboxAllDocs.enabled=false}
checkboxSaveAfter.onClick = function ()
{
    if (this.value) {dropboxFormats.enabled=true} else {dropboxFormats.enabled=false}
    if (atnXML.settings.doAtn == false && tab1.enabled == true ) {btnDo.enabled = this.value}
...
checkboxDoAtn.onClick = function ()
{
    ...
    if (atnXML.settings.saveAfter == false && tab1.enabled == true ) {btnDo.enabled = this.value}
    atnXML.settings.doAtn=this.value
}

btnDo.onClick = function ()
{
    //if(tpanel.label == 1) {w.onShow(true);tpanel.label =0}
...
{   if (this.selection !=0) {checkboxBackground.enabled = false} else {checkboxBackground.enabled = true}
    ...
    if (tpanel.label ==undefined) {tpanel.label =0} else {w.onShow(true)}
    }

checkboxEng.onClick = function (doAlert)
{
    if (doAlert!=false && checkboxRus.label == 1) alert (alertLang,XMLSettingsName)
    checkboxRus.value=false
    this.value = true
    this.label=1
    checkboxRus.label=0
...
checkboxRus.onClick = function (doAlert)
{
     if (doAlert!=false && checkboxEng.label == 1) alert (alertLang,XMLSettingsName)
     checkboxEng.value=false
     this.value = true
     this.label=1
     checkboxEng.label=0
...
{   
      try {tpanel.selection = 0} catch (e) {}
      if (renewLayers == true) {lr = sortLayers (new collectLayers ()); dropboxColor.removeAll()}
      ...
      {dropboxColor.add ("item", cptColorDropbox(lr[i].name, lr[i].layers.length, false))
       ...
       checkboxAllDocs.enabled=checkboxUseSnapshot.enabled
       ...
       var tmp =dropboxGrpName.find(atnXML.settings.setName)
        ...
        {
            case 0: checkboxRus.onClick(false) break;
            case 1: checkboxEng.onClick(false) break;
        }
...
function collectLayers ()
{
 if (app.documents.length > 0 )
 {
  ...
  var progressWindow = createProgressWindow (XMLSettingsName, msgInit, 1, lrLength+3)
  progressWindow.show();
  progressWindow.isDone = false;
   
    for (var i = 0; i<this.counter; i++)
    {progressWindow.updateProgress()
         enumLayers.push (new descLayer(lr[i]))}     
...
enumLayers = layersByColor (new layersByKind(enumLayers))
...
function buildLayersArray (objDescLayer, layersArray)
{
    progressWindow.updateProgress()
    if (isBackground==true) {if (atnXML.settings.doBackground == true) {return layersArray}} else {Color =getLayerColourByID (objDescLayer.id)}
...
this.LoadParamsFromDisk = function() {
	...
	this.InitParams = function() {
		...
		params.doAllLayers = false
         ...
         params.progressX=0
         params.progressY=0
         params.kind = 0  // 0 - графические, 1 - текстовые, 2 - группы, 3 - корректирующие слои
		return params;
	}
