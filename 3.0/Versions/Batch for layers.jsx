...
var appVersion =Number(app.version.split(".")[0])

if (appVersion>=15 && app.documents.length>0)
{
...
function buildWindow ()
{
    ...
    dropboxGrpName.onChange = function (){atnXML.setName=this.selection.text}
    ...
    sliderJpg.onChange = function () {atnXML.Jpg=this.value}
    ...
    checkboxDoAtn.onClick = function ()
    {
        ...
        if (atnXML.saveAfter == false) {btnDo.enabled = btnDo.enabled = checkboxSnapshot.enabled = this.value}
    ...
    {
        dropboxFormats.enabled= sliderJpg.enabled = checkboxIndex.enabled =this.value
        if (atnXML.doAtn == false) {btnDo.enabled = checkboxSnapshot.enabled = this.value}
        if (atnXML.useSnapshot == true) dropboxSnapshot.enabled = this.value
    ...
    dropboxFormats.onChange = function ()
    {
        ...
        switch (this.selection.index){
           case 0: sliderJpg.visible= statictextJpg.visible= statictextJpgCpt.visible=true; break;
           default: sliderJpg.visible=statictextJpg.visible=statictextJpgCpt.visible=false; break}
    }
    ...
    w.onShow = function ()
    {
            // выбираем операцию
            if (atn.length>0) {
                for (var i = 0; i < atn.length; i++) { dropboxGrpName.add ("item", atn[i].name); for (var x = 0; x < atn[i].children.length; x++) {dropboxAtnName.add ("item", atn[i].children[x].name)}}
            ...
            sliderJpg.value = atnXML.Jpg; sliderJpg.onChanging ()
    ...
    function enableControls (val)
    {
         ...
         if (atnXML.saveAfter == true || atnXML.doAtn == true) btnDo.enabled = checkboxSnapshot.enabled = val
         if (atnXML.saveAfter == true) dropboxFormats.enabled= sliderJpg.enabled = checkboxIndex.enabled = val
...
function collectLayers ()
{
    ...
    var layerCount = executeActionGet(r).getInteger(s2t('numberOfLayers'));
    var layersInfo = [],
    ...
    for (i; i <= layerCount; i++) 
    {
        ...
        kind = layerKindArray[d.getInteger(s2t ('layerKind'))],
...
function Batch ()
{
  ...
  if (atnXML.globalSnapshot == true)
  {
      ...
      if (atnXML.progress) {progressWindow.updateProgress()}
      ...
      if (appVersion>15) 
   {
      ...
      {
          if (i==counter) {if (atnXML.progress) {progressWindow.updateProgress(); counter = i+4}}
          ...
          if (curParent != parentId && cur.parent.visible== true ) {cur.parent.visible = false }
          if (batch[i].locked == true && batch[i].background == false) {cur.allLocked = false}
          if (batch[i].visible == true  && batch[i].background == false) {cur.visible = false}
        }
    }
