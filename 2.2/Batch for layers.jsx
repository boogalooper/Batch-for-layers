...
function buildWindow ()
{
    ...
    checkboxSaveAfter.preferredSize.width=185
...
checkboxIndex.onClick = function (){atnXML.settings.useIndex=this.value}
checkboxSnapshot.onClick = function (){atnXML.settings.useSnapshot=this.value; dropboxSnapshot.enabled = this.value}
dropboxSnapshot.onChange = function (){if (this.selection.index==1) {atnXML.settings.globalSnapshot = true} else {atnXML.settings.globalSnapshot = false}}

dropboxFormats.onChange = function ()
{
    atnXML.settings.saveAs=this.selection.index
    
    switch (this.selection.index){
       case 0: sliderJpg.visible=true; statictextJpg.visible=true; statictextJpgCpt.visible=true; break;
       default: sliderJpg.visible=false; statictextJpg.visible=false; statictextJpgCpt.visible=false; break}
}
...
checkboxSaveAfter.onClick = function ()
{
    ...
    if (atnXML.settings.doAtn == false) {btnDo.enabled = this.value; checkboxSnapshot.enabled = this.value, dropboxSnapshot.enabled = this.value}
...
checkboxDoAtn.onClick = function ()
{
    ...
    if (atnXML.settings.saveAfter == false) {btnDo.enabled = this.value; btnDo.enabled = this.value; checkboxSnapshot.enabled = this.value, dropboxSnapshot.enabled = this.value}
...
{   
       ...
       {
           var counter = 0
           atnXML.settings.initKind = atnXML.settings.kind
           atnXML.settings.kind = 4
           lr = sortLayers(layersByColor(layersByKind (allLayers)))
           if (lr.length >0) 
           {
               for (i=0; i<lr.length; i++) {counter =counter+ lr[i].layers.length}
           }  
            if (counter<=1) {atnXML.settings.kind = atnXML.settings.initKind }
        }
      ...
      checkboxColor.value = atnXML.settings.doAllLayers
       ...
       if (lr.length!=0) // если есть слои
       {
            checkboxColor.enabled = true 
            checkboxColor.onClick () 
            btnDo.enabled=true
            checkboxDoAtn.enabled = true; if (checkboxDoAtn.value) {dropboxGrpName.enabled = dropboxAtnName.enabled = true}
             checkboxSaveAfter.enabled =true
  //           dropboxFormats.enabled =true 
             checkboxIndex.enabled =true 
             if (checkboxDoAtn.value || checkboxSaveAfter.value)  {dropboxSnapshot.enabled=checkboxSnapshot.enabled=true}
              }
       else { 
 // если  нет слоев
               ...
               dropboxFormats.enabled =false
               checkboxIndex.enabled =false
               dropboxSnapshot.enabled=false
               checkboxSnapshot.enabled=false
}  

       if (atnXML.settings.kind != 2) {checkboxColor.text  =cptAutoSearchLrs; p1.text  =cptColorPanelLrs} else {checkboxColor.text  =cptAutoSearchGroups}
     
       if (renewLayers == true) {return}

       checkboxSaveAfter.value= atnXML.settings.saveAfter; checkboxSaveAfter.onClick ()
       dropboxFormats.selection = atnXML.settings.saveAs; dropboxFormats.onChange ()
       ...
       { dropboxGrpName.add ("item", atn[i].name); for (var x = 0; x < atn[i].children.length; x++) {dropboxAtnName.add ("item", atn[i].children[x].name)}}
               ...
               var tmp = dropboxGrpName.find(desc.getString( charIDToTypeID("PrNm"))) } catch (e) 
                {tmp = dropboxGrpName.find(atnXML.settings.setName); if (tmp == null) {tmp=0}}
               
                if (tmp!=null) {dropboxGrpName.selection = tmp} else 
                {
                    for (var i = 0; i < atn.length; i++) 
                    {
                        for (var x = 0; x < atn[i].children.length; x++) 
                        {
                            if (atn[i].children[x].name==desc.getString( charIDToTypeID("PrNm"))) {tmp = i; break}
                         }
                        if (tmp) {break}
                     }
            ...
            try {var ref = new ActionReference();  
               ref.putEnumerated( charIDToTypeID("Actn"), charIDToTypeID("Ordn"), charIDToTypeID("Trgt") );   
               var desc = executeActionGet(ref);  
              var tmp = dropboxAtnName.find(desc.getString( charIDToTypeID("Nm  ")))} catch (e) 
          {tmp = dropboxAtnName.find(atnXML.settings.atnName); if (tmp == null) {tmp=0}}   
          
          if (tmp) {dropboxAtnName.selection = tmp} 
          else {
              
              try {var ref = new ActionReference();  
               ref.putEnumerated( charIDToTypeID("Actn"), charIDToTypeID("Ordn"), charIDToTypeID("Trgt") );   
               var desc = executeActionGet(ref);  
               var tmp = dropboxAtnName.find(desc.getString( charIDToTypeID("PrNm"))) } catch (e) {}
        ...
        if (atnXML.settings.globalSnapshot) {dropboxSnapshot.selection = 1} else {{dropboxSnapshot.selection = 0}}
       ...
       checkboxIndex.value = atnXML.settings.useIndex
        if (atnXML.settings.useSnapshot) {checkboxSnapshot.value = dropboxSnapshot.enabled = true} else {checkboxSnapshot.value =  dropboxSnapshot.enabled =false}
}
...
function layersByKind (layersArray)
{
    ...
    for (var i=0; i<layersArray.length;i++)
    {
        ...
        {
            case 0:
            if (layersArray[i].kind == "LayerKind.NORMAL" || layersArray[i].kind == "LayerKind.SMARTOBJECT" || layersArray[i].kind == "LayerKind.LAYER3D") {tmp.push (layersArray[i])}
            break;
            case 1:
            ...
            break;
            case 3:
            if (layersArray[i].kind != "LayerSet" && layersArray[i].kind != "LayerKind.TEXT" && layersArray[i].kind != "LayerKind.NORMAL" && layersArray[i].kind != "LayerKind.SMARTOBJECT"  && layersArray[i].kind != "LayerKind.VIDEO") {tmp.push (layersArray[i])
            $.write (layersArray[i].kind + '\n')}
            break;
            case 4:
...
function CreateUniqueFileName( inFolder, inFileName, inExtension ) {
    ...
    if (atnXML.settings.useIndex == false)
    {
	var fileNumber = 1;
