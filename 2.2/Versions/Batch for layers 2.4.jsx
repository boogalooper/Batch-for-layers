...
{   
 ...
 // если  нет слоев
       ...
       if (!tmp) {
               ...
               tmp = dropboxGrpName.find(desc.getString( charIDToTypeID("PrNm"))) } catch (e) {}}   
       ...
       if (!tmp) {
            try {var ref = new ActionReference();  
               ref.putEnumerated( charIDToTypeID("Actn"), charIDToTypeID("Ordn"), charIDToTypeID("Trgt") );   
               var desc = executeActionGet(ref);  
               tmp = dropboxAtnName.find(desc.getString( charIDToTypeID("Nm  ")))} catch (e) {} }   
        ...
        if (atnXML.settings.useSnapshot) {checkboxSnapshot.value = true} else {checkboxSnapshot.value = false}
        if (atnXML.settings.globalSnapshot) {dropboxSnapshot.selection = 1} else {{dropboxSnapshot.selection = 0}}
        checkboxSnapshot.onClick ()
