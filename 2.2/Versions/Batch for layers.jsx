...
cptUseSnapshots={ru: "возвращать документ к исходному состоянию", en: "back document to initital state after each iteration"}
...
cptAllDocs={ru: "возвращать к исходному состоянию все документы", en: "back to initial state all opened documents"}
cptJpg={ru: "Качество jpg:", en: "Quality of jpg"}
cptFlatten={ru: "объединить слои перед сохранением", en: "flatten layers before save"}
...
function buildWindow ()
{
    ...
    var checkboxAllDocs = p3.add ("checkbox",undefined, cptAllDocs)
    checkboxAllDocs.preferredSize.width =295
    var checkboxBackground = p3.add ("checkbox",undefined, cptNotBackground)
...
dropboxFormats.onChange = function ()
{
    ...
    switch (this.selection.index){
       case 0: checkboxFlatten.enabled=false; sliderJpg.enabled=true; checkboxFlatten.value = 1; break;
       default: checkboxFlatten.enabled=true; sliderJpg.enabled=false; checkboxFlatten.value= atnXML.settings.flatten; break}
}
...
{   
      ...
      checkboxColor.value = atnXML.settings.doAllLayers
       ...
       if (lr.length == 1 && lr[0].name == "none") // если нет цветных меток, но есть слои
       {
            //checkboxColor.value = false
            checkboxColor.enabled = true 
            checkboxColor.onClick () 
            btnDo.enabled=true
            checkboxDoAtn.enabled = true; if (checkboxDoAtn.value) {dropboxGrpName.enabled = checkboxDoAtn.enabled; dropboxAtnName.enabled = checkboxDoAtn.enabled}
           ...
           if (lr.length==0) { // если нет цветных меток, и нет слоев
               checkboxColor.enabled = false 
               dropboxColor.enabled=false
               btnDo.enabled=false
               checkboxDoAtn.enabled = false; if (checkboxDoAtn.value) {dropboxGrpName.enabled = checkboxDoAtn.enabled; dropboxAtnName.enabled = checkboxDoAtn.enabled} 
           ...
           else {        // если есть цветные метки и слои
               checkboxColor.onClick ()      
               checkboxColor.enabled = true
               dropboxColor.enabled=!checkboxColor.value
               btnDo.enabled=true
               checkboxDoAtn.enabled = true; if (checkboxDoAtn.value) {dropboxGrpName.enabled = checkboxDoAtn.enabled; dropboxAtnName.enabled = checkboxDoAtn.enabled} 
               checkboxUseSnapshot.enabled=true
               checkboxAllDocs.enabled = checkboxUseSnapshot.value
